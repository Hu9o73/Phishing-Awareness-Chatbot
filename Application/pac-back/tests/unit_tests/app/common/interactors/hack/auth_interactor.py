...
import bcrypt
from app.common.base_models import OrganizationModel, OrgMemberModel, UserModel
...
class HackAuthenticationInteractor(AuthenticationInteractor):
    ...
    @staticmethod
    def create_an_org_admin(first_name: str, last_name: str, email: str, password: str, org_id: UUID):
        supabase = get_db()
        response = (
            supabase.table("users")
            .insert(
    ...
    @staticmethod
    def create_an_org_member(first_name: str, last_name: str, email: str, org_id: UUID):
        supabase = get_db()
        response = (
            supabase
            .table("org_members")
    ...
    @staticmethod
    def delete_a_member_by_id(member_id: UUID):
        supabase = get_db()
        return supabase.table("org_members").delete().eq("id", member_id).execute()
