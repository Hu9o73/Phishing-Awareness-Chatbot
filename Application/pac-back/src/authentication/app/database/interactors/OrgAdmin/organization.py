...
class OrgAdminOrganizationInteractor:
    @staticmethod
    async def get_member_by_id(member_id: UUID, org_id: UUID) -> OrgMemberModel:
        supabase: Client = get_db()
        response = supabase.table("org_members").select("*").eq("id", member_id).eq("organization_id", org_id).execute()
    ...
    @staticmethod
    async def get_members_in_organization(org_id: str) -> list[OrgMemberModel]:
        supabase: Client = get_db()
        response = supabase.table("org_members").select("*").eq("organization_id", org_id).execute()
        if not response.data or len(response.data) == 0:
            raise HTTPException(
    ...
    @staticmethod
    async def delete_member(member_id: UUID, org_id: UUID) -> StatusResponse:
        supabase: Client = get_db()
        existence_check = (
            supabase.table("org_members").select("*").eq("id", member_id).eq("organization_id", org_id).execute()
        )
