  ...
  <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click.self="$emit('close')">
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 transform transition-all">
      <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-phisward-primary">Create Organization Admin</h2>
        <button
          @click="$emit('close')"
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await fetch(`/api/authentication/admin/user?${params}`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(data.detail || 'Failed to create org admin')
    }
