  ...
  <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" @click.self="$emit('close')">
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 transform transition-all">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          <label for="member_first_name" class="block text-sm font-semibold text-gray-700 mb-2">First Name *</label>
          <input
        ...
        <div>
          <label for="member_last_name" class="block text-sm font-semibold text-gray-700 mb-2">Last Name *</label>
          <input
            id="member_last_name"
        ...
        <div>
          <label for="member_email" class="block text-sm font-semibold text-gray-700 mb-2">Email *</label>
          <input
            id="member_email"
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await fetch(`/api/authentication/orgadmin/member?${params.toString()}`, {
      method: 'POST',
      headers: {
